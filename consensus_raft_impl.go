...
func New(config *consensusUtils.ConsensusImplConfig) (*ConsensusRaftImpl, error) {
	...
	consensus.waldir = path.Join(localconf.ChainMakerConfig.GetStorePath(), consensus.chainID,
		fmt.Sprintf("%s_%s", walDir, config.NodeId))
	consensus.snapdir = path.Join(localconf.ChainMakerConfig.GetStorePath(), consensus.chainID,
...
func (consensus *ConsensusRaftImpl) publishEntries(ents []raftpb.Entry) (exit bool, configChanged bool) {
	...
	for i := range ents {
		...
		case raftpb.EntryNormal:
			...
			// unmarshal the chainMaker block from the log entry
...
// marshals and packages them to NetMsg in chainMaker,
...
func (consensus *ConsensusRaftImpl) sendMessages(msgs []raftpb.Message) {
	for _, m := range msgs {
		...
		// get the chainMaker nodeId by the etcd-raft nodeId
		...
		if !ok {
			...
			// package the marshaled data to a chainMaker NetMsg
			netMsg := &netpb.NetMsg{
				Payload: data,
...
// the nodeId in the chainMaker (string)
